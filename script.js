...
const files = [
  {
    ...
    profile:"対象は鳴響に所属する職員。爆発を操作する能力を有する。",
     weapon:"破調刀",
  ...
  },
  {
    ...
    profile:"対象は鳴響所属。主にデバイスを介して遠隔で情報を得たり、ハッキングを行う。また、破壊不可能な蒼龍を操る。",
    record:"[アクセス拒否]",
    note:"[アクセス拒否]",
  },
  {
    ...
    need:"LEVEL-4",
    remark:"PARTIAL DATA LOCKED",
    profile:"対象は鳴響所属。主に所持する懐中時計を使って味方以外の時間を一秒止める。",
    record:"[アクセス拒否]",
    note:"[アクセス拒否]",
  },
   {
...
`,
    ...
    need:"LEVEL-3",
    remark:"PARTIAL DATA LOCKED",
    profile:"対象はnu-7に所属する。かなり陽気な性格をしている。",
    record:"[アクセス拒否]",
    note:"[アクセス拒否]",
  },
   {
...
function startLoadingSequence(){const boot=document.getElementById("bootScreen");boot.style.display="block";boot.innerHTML="";const lines=["> INITIALIZING BOOT SEQUENCE...","> CHECKING HARDWARE... [OK]","> CONNECTING TO TENRI-NETWORK... [CONNECTED]","> LOADING ENCRYPTION MODULES...","> VERIFYING CLEARANCE LEVEL...","> WELCOME, ADMINISTRATOR."];let i=0;function add(){if(i<lines.length){const div=document.createElement("div");div.innerText=lines[i];boot.appendChild(div);div.scrollIntoView({behavior:"smooth",block:"end"});beep(500+i*120,40,0.03);i++;setTimeout(add,700);}else{setTimeout(finishBoot,1000);}}add();}
